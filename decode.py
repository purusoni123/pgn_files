...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab, datafiles):
    """Initialize decoder.
  ...
  def decode(self):
    ...
    if not os.path.exists(os.path.join(self._decode_dir, self._time_str)):
      os.mkdir(os.path.join(self._decode_dir, self._time_str))
  ...
  def print_and_append(self, article_withunks, abstract_withunks, decoded_words):
    # Appends summary in 'summary.txt'
    summary = ''
    for word in decoded_words:
      summary += word + ' '
    article_txt = bcolors.BOLD +"\n\n Article: \n\n" + bcolors.ENDC+ bcolors.OKGREEN + article_withunks + bcolors.ENDC
    article_txt += bcolors.BOLD +"\n\n Human Summary: \n\n" + bcolors.ENDC + abstract_withunks
    article_txt += bcolors.BOLD +"\n\n Generated Summary: \n\n" + bcolors.ENDC + bcolors.OKBLUE + summary + bcolors.ENDC
    article_txt += '\n-------------------------------------------------------------------------------\n'
    article_txt += '-------------------------------------------------------------------------------\n\n'
    tf.logging.info(article_txt)
    output_txtname = os.path.join(os.path.join(self._decode_dir, self._time_str), 'summary.txt')
    with open(output_txtname, 'a+') as f:
      article_txt = article_txt.replace(bcolors.OKBLUE,'')
      article_txt = article_txt.replace(bcolors.ENDC,'')
  ...
  def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens, json_dict):
    ...
    """
    article_idx = json_dict[article]
    # article = modify_article(article)
    ...
    # modifies article to write into "summary.txt"
    # summary = ''
    ...
    output_fname = os.path.join(os.path.join(self._decode_dir, self._time_str), 'attn_vis_data_'+str(article_idx)+'.json')
    # output_txtname = os.path.join(os.path.join(self._decode_dir, self._time_str), 'summary.txt')
...
def modify_article(article):
  ...
  # article = re.sub('[][n][\']','n\'',article)
  article = article.replace('-lrb- cnn -rrb-','')
  article = article.replace('-lrb-','')
  article = article.replace('-rrb-','')
